...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (w.toLowerCase() groupBy ((c: Char) => c)).toList.sorted map { case (x, y) => (x, y.length) }
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case x :: xs => {
      val y = combinations(xs)
      val z = for {
        sub <- combinations(xs)
        index <- 1 to x._2
      } yield ((x._1, index) :: sub)
      z ++ y
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val m = x.toMap
    (y foldLeft m)((a, b) => {
      val d = (a apply b._1) - b._2
      if (d > 0) a updated (b._1, d)
      else a - b._1
    }).toList.sorted
  }
